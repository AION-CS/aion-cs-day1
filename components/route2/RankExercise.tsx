...
/**
 * Exercise 2 — rank three of seven guiding decisions and justify the first.
 *
 * Click to add (positions fill 1 → 3), click a ranked item to remove it,
 * arrows to reorder, full undo/redo. Nothing is ever auto-sorted. "Test my
 * ranking" deliberately does not grade the order — there is no single
 * correct one — it asks one diagnostic question about the learner's own #1.
 */
              ...
              <li key={i}>
                {d ? (
                  <div className="reveal-in flex items-start gap-2 rounded-xl border border-accent/40 bg-paper p-2.5">
                    ...
                    <button type="button" onClick={() => remove(d.id)} title="Remove from the ranking" className="min-w-0 flex-1 text-left">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      <button type="button" onClick={() => move(i, -1)} aria-label={`Move ${d.label} up`} className="rounded-md border border-line p-1 text-ash hover:text-ink">
                        <ChevronDown className="h-3.5 w-3.5 rotate-180" />
                      </button>
                      <button type="button" onClick={() => move(i, 1)} aria-label={`Move ${d.label} down`} className="rounded-md border border-line p-1 text-ash hover:text-ink">
                        <ChevronDown className="h-3.5 w-3.5" />
                      </button>
                      <button type="button" onClick={() => remove(d.id)} aria-label={`Remove ${d.label}`} className="rounded-md border border-line p-1 text-ash hover:text-danger">
            ...
            return (
              <li key={d.id}>
                ...
                >
                  ...
                  <span>
                    <span className={clsx("block text-caption font-semibold", ranked ? "text-accent" : "text-ink")}>{d.label}</span>
      ...
      <div id={domId.rankCheck} className="scroll-mt-24">
        ...
        {testOpen && first && (
          <div className="reveal-in mt-2 space-y-2 rounded-xl border border-line bg-canvas p-3">
            ...
            {r2.rankCheck && (
              <p className="reveal-in text-caption text-ash">{r2.rankCheck === "required" ? check.feedbackRequired : check.feedbackOnce}</p>
            )}
            <button
              type="button"
            ...
            {clueOpen && (
              <p className="reveal-in rounded-lg border border-accent/25 bg-accentSoft px-2.5 py-1.5 text-caption text-ink">{check.clue}</p>
            )}
          </div>
        )}
      </div>
