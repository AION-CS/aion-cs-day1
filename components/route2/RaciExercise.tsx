...
/**
 * Exercise 4 — the RACI for Synervia's digital-sustainability assessment
 * standard.
 *
 * The same grid component the material taught with, now persisted. It
 * reports structure only — more than one A, no A, no R — plus the authority
 * question when an Accountable cannot bind capacity. It never names the
 * right role.
 */
export function RaciExercise() {
  ...
  return (
    <section id={domId.raci} className="scroll-mt-24 space-y-4 rounded-2xl border border-line bg-paper p-5">
      <ExerciseHeader n={RACI_EXERCISE.n} title={RACI_EXERCISE.title} minutes={RACI_EXERCISE.minutes} intro={RACI_EXERCISE.intro} material={RACI_EXERCISE.material} />
