...
/**
 * Exercise 2 — the trade-off map. Five measures, two diagnostic questions each;
 * the pair of answers decides the quadrant, and only then does the measure
 * slide onto the map (CURRICULUM-GUIDE §5: decide first, discover the position).
 *
 * Every answer change goes through the history, so undo/redo restores whole
 * map states; "Remove and retry" clears one measure's answers and re-opens it
 * for immediate re-answering (CLAUDE.md #5). The strategic-bet line is kept in
 * the store across a retry, so re-landing in that quadrant restores it.
 */
export function MapExercise() {
  ...
  const snapshot = (): PlacementMap =>
    Object.fromEntries(
      r2.mapStates.flatMap((s) => [
        [`${s.measure.id}:q1`, s.q1],
        [`${s.measure.id}:q2`, s.q2],
      ]),
    );
  ...
  return (
    <section id={domId.map} className="scroll-mt-24 space-y-4 rounded-2xl border border-line bg-paper p-5">
      <ExerciseHeader
        n={MAP_EXERCISE.n}
        title={MAP_EXERCISE.title}
        minutes={MAP_EXERCISE.minutes}
        intro={MAP_EXERCISE.intro}
        material={MAP_EXERCISE.material}
      />
...
function MeasureCard({
  ...
  /**
   * Check at the level of the measure, not the question. Both diagnostics are
   * yes/no, so saying *which* answer failed would simply be the answer
   * (CLAUDE.md #4). The learner learns that the pair does not hold up, and the
   * clue behind the second click gives a direction for both questions.
   */
  const runCheck = () => {
  ...
  return (
    <li
      ...
      className={clsx(
        "scroll-mt-24 rounded-xl border bg-paper transition-colors duration-150",
          ...
          <div>
            ...
            {verdict === "hold" && (
              <p className="reveal-in mt-2 rounded-xl border border-accent/30 bg-accentSoft px-3 py-2 text-caption text-ink">
                Both answers hold up for this measure.
              </p>
            )}
            {verdict === "wrong" && (
