...
/**
 * Step 3 — one escalated signal, analysed in depth: area, direct/indirect
 * effect, improvement approach.
 *
 * The check covers area and effect together and reports at the level of the
 * analysis, because the effect is a two-way choice (CLAUDE.md #12). When it
 ...
 * reasoning — recorded in the export. The improvement approach is free text
 * and never checked.
 */
export function DeepDiveCard({ analysis, position }: { analysis: Analysis; position: number }) {
  ...
  const runCheck = () => {
    if (!analysis.area || !analysis.effect) {
      setError(true);
      scrollToAndFlash(!analysis.area ? domId.area(signal.id) : domId.effect(signal.id));
      return;
    }
    setError(false);
    setNote(R1.analysisChecks(signal.id), String(analysis.checks + 1));
    setNote(R1.analysisLastSig(signal.id), `${analysis.area}:${analysis.effect}`);
  };
  ...
  const stale = analysis.checks > 0 && !analysis.fresh && !!analysis.area && !!analysis.effect;
  const triageLabel = analysis.triage.tag ? SENTIMENTS.find((s) => s.id === analysis.triage.tag)?.label : null;
  ...
  return (
    <article id={domId.analysis(signal.id)} className="scroll-mt-24 space-y-4 rounded-2xl border border-accent/40 bg-paper p-4">
      ...
      <MaterialRefs refs={materialRefs(["rebound", "framework"])} />
      ...
      <div id={domId.effect(signal.id)} className="scroll-mt-24">
        <p className="text-caption font-semibold text-ink">{EFFECT_FIELD.label}</p>
        <p className="mt-0.5 text-micro text-ash">{EFFECT_FIELD.instruction}</p>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {EFFECTS.map((e) => {
            const on = analysis.effect === e.id;
            return (
              <button
                key={e.id}
                type="button"
                onClick={() => choose(R1.effect(signal.id), e.id)}
                aria-pressed={on}
                className={clsx(
              ...
              >
                ...
                <span className="mt-0.5 block text-micro text-ash">{e.hint}</span>
              </button>
            );
          })}
        </div>
