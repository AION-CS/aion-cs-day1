...
import { SENTIMENTS, R1, TRIAGE } from "@/lib/route1";
import { domId, type TriageRowState } from "./useRoute1";

/**
 * One signal in the triage: a Positive/Negative tag plus the phrase in the
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div id={domId.triageTag(signal.id)} className="mt-3 flex scroll-mt-24 flex-wrap gap-2">
            {SENTIMENTS.map((sm) => {
              const on = row.tag === sm.id;
              return (
                <button
                  key={sm.id}
                  type="button"
                  onClick={() => choose(R1.triageTag(signal.id), sm.id)}
                  aria-pressed={on}
                  title={sm.hint}
                  className={clsx(
