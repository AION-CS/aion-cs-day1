...
export function ExportBar() {
  ...
  const stages = [r1.walkComplete, r1.sortComplete, r1.diagnoseComplete, r1.decideComplete];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          <span className="tabular-nums font-semibold text-ink">{doneStages}</span> / {stages.length} checks complete
          {r1.missing.length > 0 && (
            <ChevronDown
        ...
        <button type="button" onClick={handleExport} className="btn-accent">
          Export Green Workplace Diagnostic
        </button>
      </div>
    </div>
  );
