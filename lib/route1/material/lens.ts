...
/** SVG #10 demo: one illustrative option drawn from invented ranks, then cleared. */
export const LENS_DEMO: { label: string; note: string; ranks: Record<CriterionId, Rank> } = {
  ...
  note: "Illustrative ranks, drawn once and then cleared. Rank 1 plots on the outer ring.",
  ranks: {
    leverage: 2,
    sustainability: 2,
    innovation: 1,
...
export const S6_LENS: MaterialSection<MaterialSectionId> = {
  ...
  standfirst:
    "Seven criteria, seven diagnostic questions, seven common misuses — and no option that should come first on all of them.",
  minutes: 22,
  definition:
  ...
  body: [
    ...
    {
      heading: "Ranking instead of scoring",
      paragraphs: [
        "Scores on a scale invite false precision: a 7 and an 8 look like a measurement when they are a judgement. Forced ranking — 1, 2 and 3 on each criterion, no ties — keeps the judgement visible and makes you choose. On every criterion, rank 1 means the strongest option; for Risk, the strongest option is the one with the least exposure if your assumptions prove wrong.",
        "A radar drawn from ranks shows the shape of each option at a glance, and a rank sum condenses it into one number. Both summarise your own judgement; neither is a score of correctness. A low rank sum does not automatically mean the right choice, because the seven criteria are not equally important in every situation — and deciding their weight is the part of the decision you own.",
      ],
    },
    {
  ...
  reasoning: [
    "Rank each row of the Part 2 matrix against that row's diagnostic question, not its label — the questions are what separate a real assessment from one of the misuses.",
    "Rank 1 always means the strongest option on that criterion. For Risk, that is the option with the least exposure if your assumptions prove wrong — not the one with the most proven technology.",
    "If one option comes first on all seven criteria, one of the misuses is at work. Re-read your Risk and Feasibility rows against the stated budget constraint.",
  ],
  callout: {
