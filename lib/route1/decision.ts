/**
 * Route 1, stage 2 — the deciding half of the AppNexa engagement (level 2).
 *
 * The constraint the quarter actually runs under, three competing lines of
 * measures mapped onto the Green Software Patterns lifecycle, the seven
 * dimensions they are judged on, and what makes a call defensible when the
 * telemetry to prove it does not exist yet.
 *
 * Sections are lettered G–J because they continue stage 1's run rather than
 * restarting it (CLAUDE.md #12). The standalone SCI / six-areas recap this half
 * used to open with was deleted in the merge: it existed only so the old
 * Route 2 could be entered cold, and inside one continuous route it reads as a
 * restart of material the learner finished twenty minutes ago. What it carried
 * that still matters — which of the six areas each option attacks — now lives
 * in the bridge between the two stages (see BRIDGE in ./index.ts).
 */
...
// ---------------------------------------------------------------------------
// Material, sections G–J — the second half of the route's continuous run.
// ---------------------------------------------------------------------------
export const DECISION_MATERIAL: MaterialSection[] = [
  {
    id: "constraint",
    n: 7,
    letter: "G",
    icon: "coins",
    kicker: "G · What actually limits this decision",
  ...
  },
  {
    ...
    letter: "H",
    icon: "layers",
    kicker: "H · Three options, three lifecycle stages",
  ...
  },
  {
    ...
    letter: "I",
    icon: "target",
    kicker: "I · The seven axes you will score on",
  ...
  },
  {
    ...
    letter: "J",
    icon: "shield",
    kicker: "J · The skill this stage is actually testing",
...
export const DECISION_READING = {
  heading: "Further reading — sections G–J",
  intro: "The two primary sources behind this half's framing of the options.",
  items: [
    {
...
// ---------------------------------------------------------------------------
// Stage 2 framing and commit fields. Same rule as stage 1: the case, the name
// field and the export live once, in ./index.ts.
// ---------------------------------------------------------------------------
export const STAGE2 = {
  id: "stage2",
  tag: "STAGE 2",
  title: "Spend the quarter",
  framing:
    "Management will fund exactly one of these lines of measures: A) Binding Green Coding Guidelines, B) Technical Rework of the Worst Offenders, or C) Development Controlling & Observability. For each one, answer the situational question, predict its profile across the seven decision dimensions, then reveal the real profile — the gap between your dashed prediction and the solid real polygon is the part worth reading. Compare all three as often as you like, in any order. Then commit to one and defend it, including the two risks of the road you did not take.",
  predictInstruction:
